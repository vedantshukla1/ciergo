...
import { ChevronDown, ClipboardList, Plus, IndianRupee, ArrowRightLeft, ArrowUpDown, X, Check } from 'lucide-react';
...
import { EXACT_OWNERS, WAITING_ROWS } from '@/data/bookingTableMockData';
...
export const ApprovalBookingsTable = ({ isSelectMode, selectAllTrigger, clearSelectionTrigger, searchQuery }: BookingTableProps) => {
  ...
  let currentRows = [...(WAITING_ROWS as unknown as any[])];
  ...
  let showingText = `Showing 1-${Math.min(6, currentRows.length)} of ${currentRows.length} Bookings`;
    ...
    <div className="flex-1 flex flex-col p-4 bg-white">
                    ...
                    <div className="flex items-center justify-center gap-3">
                      <div className="flex items-center justify-center gap-2 w-[72px]">
                        {row.needsApproval ? (
                          <>
                            <button 
                              onClick={() => setModalConfig({ isOpen: true, type: 'approve', bookingId: row.bookingId })}
                            ...
                            <button 
                              onClick={() => setModalConfig({ isOpen: true, type: 'reject', bookingId: row.bookingId })}
                      ...
                      <RowActionsDropdown hideDelete={false} isEmptyState={row.emptyState} hideLink={row.needsApproval} />
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {modalConfig && (
        <ConfirmationModal
          ...
          onConfirm={() => {
            console.log(`${modalConfig.type} confirmed for ${modalConfig.bookingId}`);
          }}
          type={modalConfig.type}
          title={
            <>
              Are you sure you want to {modalConfig.type} this booking with<br/>
              ID <span className="font-bold text-gray-900">'{modalConfig.bookingId}'</span> ?
            </>
          }
